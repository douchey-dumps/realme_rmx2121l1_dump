...
SPECIAL_CUSTOM_OPEN_PLATFORM_RES_PATH=/my_custom/media/logo
...
function choose_special_logo_file {
    ...
    if [[ x"${special_logo_file}" == x"" && -d ${SPECIAL_CUSTOM_OPEN_PLATFORM_RES_PATH} ]];then
        if [ -d ${SPECIAL_CUSTOM_OPEN_PLATFORM_RES_PATH}/${lcm_height} ];then
            for bmp_file in `find ${SPECIAL_CUSTOM_OPEN_PLATFORM_RES_PATH}/${lcm_height} -name "*.bmp"`;do
                special_logo_file=${bmp_file}
                break
            done
        fi
